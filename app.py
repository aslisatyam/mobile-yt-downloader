...
if st.button("🚀 Prepare Download Link", use_container_width=True):
    if not url.strip():
        ...
        with st.spinner("⏳ Link taiyar ho rahi hai... 403/Bot block bypass kiya jaa raha hai..."):
            ...
            data = {
                "url": url.strip(),
                "vQuality": "720",  # High resolution support bina crash kiye
                "isAudioOnly": is_mp3
            }
            ...
            try:
                ...
                if response.status_code == 200 or res_data.get("status") == "redirect":
                    ...
                    st.success("✅ Download link ekdum taiyar hai!")
                else:
                    st.error(f"❌ Server error: {res_data.get('text', 'Bhai firse try karo')}")
            except Exception as e:
                st.error(f"❌ Connection fail ho gaya: {str(e)}")

# Green rang ka mast button browser download trigger karne ke liye
