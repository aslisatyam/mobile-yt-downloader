...
# Title Elements (Jaise aapke GUI mein tha)
...
# Session state initialize karna data save rakhne ke liye
...
if 'status_msg' not in st.session_state:
    st.session_state.status_msg = ("Ready to roll! 😎", "gray")

# Input URL Box
url = st.text_input("Paste Video/Playlist Link Here:", placeholder="https://youtube.com...")

# 1. FETCH INFO BUTTON (Jaise aapke code ka fetch_info tha)
            ...
            try:
                with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                    ...
                    if is_playlist:
                        ...
                        st.session_state.status_msg = ("✅ Playlist mil gayi! Direct download par click karein.", "green")
                        
                        entries = list(info.get('entries', []))
                        st.session_state.thumbnail_url = entries[0].get('thumbnail') if entries and entries[0] else None
                    else:
                        # Exact aapka video quality filtering logic
                        ...
                        if sorted_qualities:
                            ...
                            st.session_state.status_msg = ("✅ Qualities mil gayi! Format select karke download karein.", "green")
                        else:
                            st.session_state.qualities_list = ["Best Quality (Auto)"]
                        
                        st.session_state.thumbnail_url = info.get('thumbnail')
            except Exception as e:
                st.session_state.status_msg = (f"❌ Info fetch fail ho gayi! {str(e)}", "red")

# Status message dikhane ke liye
st.markdown(f"<p style='color: {st.session_state.status_msg[1]}; font-style: italic;'>{st.session_state.status_msg[0]}</p>", unsafe_allow_html=True)

# Quality Selection Menu (Dropdown)
selected_quality = st.selectbox("Select Video Quality:", st.session_state.qualities_list)

# Thumbnail Display (Aapke GUI ki tarah exact placement)
...
# 2. DOWNLOAD GENERATION LOGIC
# Web apps mein server par file download karke user ko link di jaati hai
if url.strip() and st.session_state.status_msg[1] == "green":
        ...
        with st.spinner("⏳ Server par file taiyar ho rahi hai, thoda time lag sakta hai..."):
            try:
                if is_mp3:
                    ...
                    # Agar specific quality select ki hai toh aapka format_id selector lagaya
                ...
                # Cloud par cloud binaries automatic merge handles safely
                with yt_dlp.YoutubeDL(download_opts) as ydl_dl:
                    ydl_dl.download([url.strip()])
                
                # Jab file server par ban jaye, user ko browser/mobile mein save karne ka button dena
